...
#include "left_liftdrag_plugin.h"

using namespace gazebo;

GZ_REGISTER_MODEL_PLUGIN(left_liftdrag_plugin)

/////////////////////////////////////////////////
left_liftdrag_plugin::left_liftdrag_plugin() : cla(1.0), cda(0.01), cma(0.01), rho(1.2041)
{
...
/////////////////////////////////////////////////
left_liftdrag_plugin::~left_liftdrag_plugin()
{
}

/////////////////////////////////////////////////
void left_liftdrag_plugin::Load(physics::ModelPtr _model,
                     sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_model, "left_liftdrag_plugin _model pointer is NULL");
  GZ_ASSERT(_sdf, "left_liftdrag_plugin _sdf pointer is NULL");
  this->model = _model;
  this->sdf = _sdf;

  this->world = this->model->GetWorld();
  GZ_ASSERT(this->world, "left_liftdrag_plugin world pointer is NULL");
...
#endif
  GZ_ASSERT(this->physics, "left_liftdrag_plugin physics pointer is NULL");
  ...
  if (_sdf->HasElement("a0"))
    this->alpha0 = _sdf->Get<double>("a0");//初始迎角

  if (_sdf->HasElement("cla"))
    this->cla = _sdf->Get<double>("cla");//升力系数/迎角的斜率

  if (_sdf->HasElement("cda"))
    this->cda = _sdf->Get<double>("cda");//阻力系数/迎角的斜率

  if (_sdf->HasElement("cma"))
    this->cma = _sdf->Get<double>("cma");//力矩系数/迎角的斜率

  if (_sdf->HasElement("alpha_stall"))
    this->alphaStall = _sdf->Get<double>("alpha_stall");//失速迎角

  if (_sdf->HasElement("cla_stall"))//
    this->claStall = _sdf->Get<double>("cla_stall");//失速后升力系数/迎角的斜率

  if (_sdf->HasElement("cda_stall"))
    this->cdaStall = _sdf->Get<double>("cda_stall");//失速后阻力系数/迎角的斜率

  if (_sdf->HasElement("cma_stall"))
    this->cmaStall = _sdf->Get<double>("cma_stall");//失速后力矩系数/迎角的斜率

  if (_sdf->HasElement("cp"))
    this->cp = _sdf->Get<ignition::math::Vector3d>("cp");//本体坐标系中的压力中心

  // blade forward (-drag) direction in link frame
  if (_sdf->HasElement("forward"))
    this->forward = _sdf->Get<ignition::math::Vector3d>("forward");//前向向量
  this->forward.Normalize();

  // blade upward (+lift) direction in link frame
  if (_sdf->HasElement("upward"))
    this->upward = _sdf->Get<ignition::math::Vector3d>("upward");//向上向量
  this->upward.Normalize();

  if (_sdf->HasElement("area"))//舵面面积
    this->area = _sdf->Get<double>("area");

  if (_sdf->HasElement("air_density"))//空气阻力
  ...
  if (_sdf->HasElement("link_name"))//指向当前链接关节对应目标（即机体）的指针
  {
    ...
    if (!this->link)
    {
      gzerr << "Link with name[" << linkName << "] not found. "
        << "The left_liftdrag_plugin will not generate forces\n";
    }
    else
    {
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          boost::bind(&left_liftdrag_plugin::OnUpdate, this));
    }
  }

  if (_sdf->HasElement("control_joint_name"))//指向激活控制面的关节（即作用的舵面的连接关节）的指针
  {
    std::string controlJointName = _sdf->Get<std::string>("control_joint_name");
    this->controlJoint = this->model->GetJoint(controlJointName);

    std::string filename=controlJointName + ".txt";
    gzdbg << filename<< std::endl;
  ...
  if (_sdf->HasElement("control_joint_rad_to_cl"))//舵面改变每弧度，cl的改变量
...
/////////////////////////////////////////////////
void left_liftdrag_plugin::OnUpdate()
{
  GZ_ASSERT(this->link, "Link was NULL");
  // get linear velocity at cp in inertial frame
#if GAZEBO_MAJOR_VERSION >= 9
  ignition::math::Vector3d vel = this->link->WorldLinearVel(this->cp);  //获得世界坐标系下压力中心处的速度
#else
...
if (vel.Length() <= 0.01)  return;
  ...
  // pose of body
#if GAZEBO_MAJOR_VERSION >= 9
  ignition::math::Pose3d pose = this->link->WorldPose();//获得机体的位置
#else
 ...
 force = ignition::math::Vector3d(cd * q * this->area,0,cl * q * this->area)/ 0.9491;
 force.Correct();
  link->AddRelativeForce(force);
  //link->AddForceAtRelativePosition(force, this->cp);
  torque = ignition::math::Vector3d(cl*q*this->area*cp.Y(), -cl*q*this->area*cp.X(),cd*q*this->area*cp.Y());
 // torque = ignition::math::Vector3d(cl*q*this->area*cp.Y(), 0,cd*q*this->area*cp.Y());
  torque.Correct();
 link->AddRelativeTorque(torque);

  ignition::math::Vector3d all_force = link->WorldForce();
#if GAZEBO_MAJOR_VERSION >= 9
  ...
  {
     outputfile << torque[0] << " " << torque[1] << " " << torque[2] << " "
                << force[0] << " " << force[1] << " " << force[2] << " "
                << alpha << " " << cl  << " " << cd << " " << cosSweepAngle << " "
                <<acos(cosAlpha) << " " << controlAngle << " " << now.Double() <<std::endl;
  } else
  {
