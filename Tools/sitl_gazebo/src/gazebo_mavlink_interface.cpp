...
void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  {
      std::string linkName = _sdf->GetElement("baselink")->Get<std::string>();
      link_ = model_->GetLink(linkName);
  }


    outputfile.open ("roll.txt");
}
...
void GazeboMavlinkInterface::handle_message(mavlink_message_t *msg, bool &received_actuator)
{
  ...
  case MAVLINK_MSG_ID_HIL_ACTUATOR_CONTROLS:
    ...
    for (int i = 0; i < input_reference_.size(); i++) {
      ...
      if ( i < 6 ) {
        outputfile <<  controls.controls[input_index_[i]] <<  " ";
      }
    }
      outputfile << last_actuator_time_.Double() << std::endl;
